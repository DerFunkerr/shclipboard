...
function updateClock() {
    ...
    // Holt die aktuelle Stunde, Minute und Sekunde und stellt sicher, dass sie zweistellig sind
    const hours = now.getHours().toString().padStart(2, "0"); 
    // Falls eine Zahl einstellig ist, wird eine Null davor gesetzt
   ...
   async function fetchUsername() {
       // Holt das HTML-Element mit der ID "username", in dem der Benutzername angezeigt wird
    const usernameField = document.getElementById("username");
    try {
    // Sendet eine POST-Anfrage an den Server, um den Benutzernamen abzurufen
    const response = await
   fetch('http://192.168.0.121/api/auth.php', {
    method: 'POST',
    headers: { 
    ...
    // Aktion, die auf dem Server ausgeführt wird
    }),
    });
   ...
   'Unbekannter Fehler'}`);
    ...
    // Falls ein Netzwerkfehler auftritt, wird eine Fehlermeldung ausgegeben
    console.error('Ein Fehler ist beim Abrufen des Benutzernamens aufgetreten:', error);
   ...
   async function logout() {
    try {
    // Sendet eine POST-Anfrage an den Server, um den Benutzer auszuloggen
    const response = await
   fetch('http://192.168.0.121/api/auth.php', {
    method: 'POST',
    headers: {
    ...
    const data = await response.json();
    // Falls die Abmeldung erfolgreich ist, wird eine Meldung angezeigt und der Benutzer weitergeleitet
    if (response.ok) {
    showInfo('Logout erfolgreich! Weiterleitung...');
    setTimeout(() => {
    window.location.href = '/login.php'; // Weiterleitung zur Login-Seite nach 2 Sekunden
    }, 2000);
    } else {
    // Falls ein Fehler auftritt, wird eine Fehlermeldung ausgegeben
    console.error('Fehler beim Logout:', data.error || 'Unbekannter Fehler');
    alert(`Fehler beim Logout: ${data.error || 'Unbekannter Fehler'}`);
    }
    } catch (error) {
   ...
   // Sobald die Webseite vollständig geladen ist, wird der folgende Code ausgeführt
   document.addEventListener('DOMContentLoaded', () => {
    ...
    event.preventDefault(); // Verhindert das Standardverhalten des Buttons (z. B. das Absenden eines Formulars)
    try {
